...
Change configuration of the running viewer from Python. The changes will be applied immediately.

`set_viewer_config` acts on the objects currently on screen, so it requires a model to be shown: before the first `show` or after `show_clear()` the call is not applied (the viewer logs "no model on screen"). Use `set_defaults(...)` to configure what the next `show` will use instead.
...
| `collapse` | ([Collapse](enums.md#collapse)) `Collapse.LEAVES`: collapse all single leaf nodes, `Collapse.ROOT`: expand root only, `Collapse.ALL`: collapse all nodes, `Collapse.NONE`: expand all nodes |
| `explode` | (boolean) Turn on explode mode |
| `analysis_tool` | ([AnalysisTool](enums.md#analysistool) or string) Activate an analysis tool (mutually exclusive with `explode=True`): `AnalysisTool.PROPERTIES`, `AnalysisTool.DISTANCE`, `AnalysisTool.SELECT`, `AnalysisTool.OFF` (or "properties", "distance", "select", "off") |
...
| `states` | (dict) Set the visibility state of cad objects. Format: `{'path': [1, 1]}`.<br>`[1,1]` → `[0,1]` (first element of the tuple) toggles faces, `[1,1]` → `[1,0]` (second element of the tuple) toggles edges |
| `tab` | (string/[UiTab](enums.md#uitab)) Select a tab in the viewer: "tree" \| "clip" \| "zebra" \| "material" \| "studio" (or use `UiTab.TREE`, `UiTab.CLIP`, `UiTab.ZEBRA`, `UiTab.MATERIAL`, `UiTab.STUDIO`) |
...
| Keyword | Description |
| ------- | ----------- |
| `studio_environment` | ([StudioEnvironment](enums.md#studioenvironment) or string) Environment HDR map; use the StudioEnvironment enum or a custom HDR URL |
...
| `studio_background` | ([StudioBackground](enums.md#studiobackground)) `ENVIRONMENT`, `TRANSPARENT`, `GRADIENT`, `GRADIENT_DARK`, `WHITE`, `GREY`, `DARKGREY` |
| `studio_tone_mapping` | ([StudioToneMapping](enums.md#studiotonemapping)) `NEUTRAL`, `ACES`, `NONE` |
...
| `studio_texture_mapping` | ([StudioTextureMapping](enums.md#studiotexturemapping)) `TRIPLANAR` or `PARAMETRIC` |
...
| `viewer` | The sidecar to address, by title (jupyter_cadquery) |
