...
| A Makefile target says `command not found` for `python` or `pytest`           | The run's `PATH` starts with the environment's own `bin`; check the `PATH:` line in the Backend tab. On Windows, recipes written for a Unix shell (`rm -rf`, `find`, `$$PATH`) run through `cmd.exe` unless `sh.exe` is on `PATH` |
| A `make` entry is missing from a Makefile's menu                              | `make` is not on the `PATH` a run gets — see the `PATH:` line in the Backend tab. On macOS it comes with the Xcode command line tools; on Windows from Chocolatey, Scoop or MSYS2                                                 |
