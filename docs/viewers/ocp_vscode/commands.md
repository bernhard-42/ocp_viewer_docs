...
## build123d snippets (experimental)

The command _OCP CAD Viewer: Install CAD snippets into `<project>/.vscode/`_ writes a `.code-snippets` file for build123d. After typing `bd_`, a list of snippets appears that guides through creating basic build123d patterns (prefixes `bd_p`, `bd_s`, `bd_l`, …).

The snippet bodies come from the `OcpCadViewer.snippets.dotVscodeSnippets` setting — override it in your user or workspace settings to add your own or change the existing ones. The structure mirrors a standard VS Code `.code-snippets` JSON file, grouped by library name at the top level.
