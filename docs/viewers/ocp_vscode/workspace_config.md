...
| `quickstartCommands` | object | see [Installation](installation.md) | Commands run by the _Quickstart build123d_ / _Quickstart CadQuery_ buttons |
| `installCommands` | object | see [Installation](installation.md) | Per-library install/upgrade commands run by the Library Manager |
...
Placeholder substitution (`{python}`, `{pip-install}`, `{unset_conda}`, `{ocp_vscode_version}`) is documented in [Installation](installation.md).
