...
Every `OcpCadViewer.*` setting can be edited from the VS Code Settings UI (search for "OCP CAD Viewer", or the gear button in the Viewer Manager) or directly in `settings.json`. Workspace settings override user settings. These settings are the viewer-settings tier of the [config system](../../config.md): the starting values that `set_defaults()` overrides per Python process and show keywords override per call.
...
| `new_tree_behavior` | boolean | `true`            | Eye icon controls the whole object, mesh icon only the wireframe — see [Tree](../../tabs.md#tree)                                  |
| `theme`             | enum    | `browser`         | `browser` / `light` / `dark`                                                                                                       |
| `orbit_control`     | boolean | `false`           | `orbit` control mode instead of `trackball`                                                                                        |
| `up`                | enum    | `Z`               | Up direction: `Z` / `Y`                                                                                                            |
...
| `grid_font_size`    | number  | `12`              | Font size of the grid axis labels                                                                                                  |
| `collapse`          | enum    | `leaves`          | Tree initial state: `none` / `leaves` / `all` / `root`                                                                             |
...
| Setting              | Type    | Default                                                                              | Description                                                                                                                  |
| -------------------- | ------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| `watchCommands`      | string  | `from ocp_vscode import show_all, get_port; ...`                                     | Command run by the debugger on every step; must call `show_all(locals(), ...)` — see [Visual debugging](visual_debugging.md) |
...
| `autostartTriggers`  | array   | `["from ocp_vscode import", "import ocp_vscode", "from ocp_viewer_core.viewer import", "import ocp_viewer_core.viewer"]` | Lines that, when seen in an open Python file, trigger autostart; a customized list replaces the default, see [Migration from 4.0](migration.md#autostart-with-ocp_viewer_coreviewer) |
...
| `quickstartCommands` | object  | see [Customizing the install commands](managers.md#customizing-the-install-commands) | Commands run by the _Quickstart build123d_ / _Quickstart CadQuery_ buttons                                                   |
| `installCommands`    | object  | see [Customizing the install commands](managers.md#customizing-the-install-commands) | Per-library install/upgrade commands run by the Library Manager                                                              |
| `codeSnippets`       | object  | see [Customizing the paste commands](managers.md#customizing-the-paste-commands)     | Python import snippets pasted by the "paste" button next to each library                                                     |
...
Placeholder substitution (`{python}`, `{pip-install}`, `{unset_conda}`, `{ocp_vscode_version}`) is documented in [Placeholders](managers.md#placeholders).

## OcpCadViewer.snippets.\*

| Setting             | Type   | Default                  | Description                                                                                                                 |
| ------------------- | ------ | ------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| `dotVscodeSnippets` | object | (build123d snippet pack) | Snippets written into `<project>/.vscode/` by the "Install CAD snippets" command — see [Commands and snippets](commands.md) |
